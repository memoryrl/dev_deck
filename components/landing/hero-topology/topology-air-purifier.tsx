      ...
      {angles.map((angle, i) => (
        <group key={i} rotation={[0, angle, 0]}>
          <mesh position={[HEAD_R * 0.5, HEAD_H * 0.4, 0]} rotation={[0.22, 0, 0.2]}>
            ...
            <meshStandardMaterial color="#0c0c0e" roughness={0.35} metalness={0.35} />
          </mesh>
        </group>
      ))}
    </>
    ...
    <>
      {Array.from({ length: RING_COUNT }, (_, i) => (
        ...
        >
          ...
          <meshBasicMaterial color="#eaf6ff" transparent opacity={0} depthWrite={false} side={DoubleSide} />
        </mesh>
      ))}
    </>
  )
  ...
  return (
    <group position={position}>
      ...
      <mesh position={[0, FOOT_H + BODY_H / 2, 0]}>
        ...
        <meshStandardMaterial color="#f4f2ec" roughness={0.3} metalness={0.03} />
      </mesh>
      {/* 목 트림 — 바디와 헤드 경계선 */}

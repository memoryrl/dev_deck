...
랜딩 히어로는 **클래식 카피**와 **3D 오피스 토폴로지**를 오갑니다. 오른쪽 화살표를 누르면 책상 위의 로봇이 모듈을 안내하고, 책상을 클릭하면 그 영역의 최신 글로 이어집니다. 화면 아래에서는 PromptKit · CareerLog · Steam Tracker가 숫자 줄, 스킬 칩, 라이브러리 커버로 이어집니다.
...
| 모듈 | 경로 | 무엇을 보나요 |
| --- | --- | --- |
| **PromptKit** | `/p/[id]`, `/b/prompts` | 재사용 가능한 바이브 코딩 템플릿. 상세에서 본문을 읽고 한 번에 복사합니다. |
...
카테고리와 태그를 붙여 저장하고, 본문은 CKEditor 5로 씁니다. 예전 Markdown 글도 보기 모드에서 깨지지 않습니다. 공개 글의 공유 링크(`/p/[id]`)는 목록에서 내려가도 살아 있습니다.

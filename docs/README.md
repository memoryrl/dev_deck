...
랜딩은 클래식 히어로와 3D 오피스 토폴로지를 오갑니다. 오피스의 책상은 사이트 메뉴이고, 로봇은 그 모듈의 안내자입니다. 그 아래로 PromptKit · CareerLog · Steam Tracker가 공개 쇼케이스로 이어집니다.

UI 크롬은 한국어와 English를 쿠키·쿼리로 유지합니다. 글·댓글·메뉴 본문은 자동 번역하지 않고 작성 언어를 그대로 둡니다. 라이트가 기본이고, 다크는 토글입니다.

소유자는 Google로 들어와 PromptKit, CareerLog, Steam, 게시판, 메뉴, 업로드를 같은 대시보드에서 다룹니다. 그 외 로그인은 회원(`/account`)이며 글을 쓰지 못합니다.

...
랜딩(`/`) 히어로 우측 상단에 스위치를 달아, 기본값(OFF)은 현재 히어로(`app/(home)/page.tsx` + `HeroVisual`)를 그대로 보여주고, ON이면 히어로를 풀블리드 3D 오피스 다이오라마로 교체한다.
...
[히어로 섹션 — 스위치 OFF]
┌─────────────────────────────────────────────┐
│ Personal Developer Hub          [클래식 ⟷ 토폴로지] │ ← 스위치, 우측 상단
│ 프롬프트, 커리어, 게임을...                          │
│ ...버튼들...                                       │
│                                   (카드 스택 비주얼)  │
└─────────────────────────────────────────────┘

스위치 ON ↓ (같은 섹션 영역이 풀블리드 오피스 씬으로 교체됨)
┌───────────────────────────────────────────────────┐ ← 뷰포트 좌우 끝까지
│                     [PromptKit]                     │
│         책상+로봇        책상+로봇       책상+로봇     │
│    (컴퓨터, 45도 고정 아이소메트릭 뷰, 드래그 없음)      │
│                                                       │
│         책상 클릭 → 카메라가 그 책상으로 확대           │
│    ┌─────────────────────────────────┐              │
│    │ PromptKit          ✕             │ ← 하단 슬라이드업 │
│    │  · Robot Hero                    │    (클릭한 모듈의) │
│    │  · ...                           │    게시물 목록    │
...
- 기본값은 **항상 클래식**(OFF). 스위치는 `localStorage`(`devdeck:landing-hero-view`)에 마지막 선택을 저장해 재방문 시 복원한다 — 단, 최초 진입은 항상 클래식이라는 요구사항을 지키기 위해 **서버 렌더링은 항상 클래식으로 하고, 클라이언트 mount 후에만 저장된 값이 있으면 토폴로지로 전환**한다 (깜빡임 감수, hydration mismatch 방지).
- 스위치 라벨: "클래식 · 토폴로지" 텍스트를 스위치 좌우에 배치 (아이콘만으로는 의미 불명확).
- 토폴로지 진입 시 히어로의 배경 그라디언트와 `HeroVisual` 카드 스택, 헤드라인/서브카피/CTA 버튼은 모두 숨기고 오피스 씬을 풀블리드로 표시한다.
- 아무 책상도 선택하지 않은 기본 상태에서는 좌상단에 "책상을 클릭해 보세요" 힌트 텍스트를 띄운다.

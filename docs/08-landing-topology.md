...
- **안내 퇴장(메뉴·하위 메뉴 선택 시)**: 팝업의 메뉴 링크·하위 항목·자세히 보기를 누르면 바로 이동하지 않는다. 그 책상의 로봇이 "따라오세요!"(`landing.robotFollowMe`) 말풍선을 띄우고 스툴에서 뛰어내려 옆벽의 출입문(`topology-door.tsx`)까지 걸어간다(`topology-escort-robot.tsx`, 팀원은 통로→앞 복도→문, 팀장은 책상 왼쪽 끝→파티션 앞 복도→옆벽 통로→문). 문이 열리고 로봇이 페이드아웃하면 화면 전체 베일이 덮이고 `router.push`로 이동한다. 카메라는 그동안 로봇을 따라간다. 수식키·새 탭 클릭과 `prefers-reduced-motion`은 링크 기본 동작을 유지하고, "바로 이동" 버튼으로 연출을 건너뛸 수 있다.
- **헤더·푸터 메뉴도 같은 연출**: `lib/landing/escort-bus.ts`가 내비게이션 링크와 패널을 잇는다. 토폴로지 슬라이드가 보이는 동안 패널이 핸들러를 등록하고, 헤더(데스크톱 메가 메뉴·모바일 드로어)와 푸터 컬럼 링크는 `escortLinkClick`으로 연출을 요청한다. 링크는 메뉴 id → href 완전 일치 → 경로 최장 일치 → 라벨 순으로 책상(로봇)에 매칭되고, 푸터처럼 화면 아래에서 눌렀으면 히어로를 뷰포트로 스크롤한 뒤 시작한다. 이미 안내 중이면 추가 클릭은 삼킨다.
- **토폴로지가 화면에 없어도 안내(오버레이)**: 패널이 없는 화면(다른 페이지, 클래식 슬라이드)에서는 헤더에 상주하는 `escort-overlay.tsx`가 예비 핸들러(`registerEscortHandler(…, { fallback: true })`)로 대신 맡는다. `body`로 포털한 전체 화면 레이어(z-80)에 같은 `TopologyScene`을 escort 모드로 띄워 로봇이 문 밖으로 나간 뒤 베일→`router.push`로 이동하므로, 목적지 스켈레톤 UI보다 안내 연출이 먼저 나온다. 방 데이터는 헤더의 내비 트리(`NavNode`)로 `lib/landing/topology-modules.ts`의 `topologyFromNavNodes`가 만들어 랜딩과 같은 책상 배치·로봇 배정을 쓴다(타입과 순수 빌더는 이 파일, Supabase 조립은 `topology.ts`). 주소가 바뀌면 자동으로 걷히고, 현재 주소와 같은 링크는 연출 없이 지나가며, 씬 로드 실패에 대비한 10초 안전 타임아웃과 유휴 시 three.js 청크·로봇 모델 예열이 있다.
- **메뉴 외 모든 화면 전환도 안내**: 오버레이가 `document` 캡처 단계에 `escortAnyLinkClick`을 걸어 본문 카드·버튼형 링크 등 모든 내부 `<a>` 클릭을 가로챈다(Next `Link`는 `defaultPrevented`면 자체 이동을 건너뛰므로 캡처에서 먼저 막아야 한다). 이때는 `strict` 요청으로 보내 메뉴 id → href → 경로 → 라벨로 맡을 로봇이 분명할 때만 안내하고, 못 찾으면(`/login` 등) 연출 없이 평소처럼 이동한다. 말풍선 목적지 이름은 토폴로지 데이터의 메뉴·하위 항목 라벨을 우선 쓴다(`escortTargetLabel`). 헤더·모바일 드로어·푸터·토폴로지 패널은 스스로 요청하므로 `data-escort-handled`로 표시해 두 번 처리하지 않는다. 같은 경로 안의 이동(페이지네이션·필터·해시), 새 탭·다운로드·외부 링크, 수식키 클릭, `prefers-reduced-motion`은 건드리지 않는다. `router.push`로 직접 이동하는 코드(폼 제출 후 리다이렉트 등)는 대상이 아니다.

...
- **헤더·푸터 메뉴도 같은 연출**: `lib/landing/escort-bus.ts`가 내비게이션 링크와 패널을 잇는다. 토폴로지 슬라이드가 보이는 동안 패널이 핸들러를 등록하고, 헤더(데스크톱 메가 메뉴·모바일 드로어)와 푸터 컬럼 링크는 `escortLinkClick`으로 연출을 요청한다. 링크는 메뉴 id → href 완전 일치 → 경로 최장 일치 → 라벨 순으로 책상(로봇)에 매칭되고, 푸터처럼 화면 아래에서 눌렀으면 히어로를 뷰포트로 스크롤한 뒤 시작한다. 패널이 없는 화면(다른 페이지, 클래식 슬라이드)이나 이미 안내 중이면 링크가 평소처럼 바로 이동한다.

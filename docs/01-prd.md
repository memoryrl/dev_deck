...
- 그 외 로그인 사용자는 회원이다. `/account` 마이페이지로 가며 게시물 편집 UI·액션·RLS 쓰기가 막힌다. 회원은 마이페이지에서 탈퇴할 수 있다. 관리자 계정은 탈퇴할 수 없다.
...
- `/games/top`은 플레이타임 Top 10 시상식 토폴로지(금·은·동 + 4–10등)다. 메뉴 **추천 게임**이 여기로 연결된다.
...
외부 API 키 없이 이 맥의 Ollama 모델을 쓴다. 인앱 프롬프트 실행(PK-06, v2)과는 별개다. 동작은 [15-llm.md](./15-llm.md).

| ID | 기능 | MVP |
| --- | --- | --- |
| LL-01 | 관리자 테스트 채팅 `/site/ollama-chat` | Yes |
| LL-02 | 포트폴리오 안내원: 로그인 회원이 공개 커리어·스킬·프롬프트를 근거로 질문. 질문·답변 기록, 관리자 열람 | Yes |
| LL-03 | 게시판 글쓰기 AI 템플릿: 고정 목록 + AI가 칸 내용 생성, 결과 캐시 | Yes |
| LL-04 | 답변 근거 없는 링크 차단, 사용자별 레이트리밋 | Yes |
| LL-05 | 스트리밍 응답, 게시판 외 폼(커리어·프롬프트·Steam)의 AI 템플릿 | No |
